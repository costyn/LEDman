...
void setTaskCallback() {
  ...
  if ( strcmp(routines[ledMode], "fire2012") == 0  ) {
    ...
    taskShowPattern.setInterval(40);
  } else
...
void monitorJoystick() {
  static bool xThreshold = false ;
  static bool yThreshold = false ;

  int xAxis = analogRead( X_AXIS ) ;
  int yAxis = analogRead( Y_AXIS ) ;
  ...
  if( xAxis > X_UPPER && ! xThreshold ) {
    nextPattern() ;
    Serial.println("Next");
    xThreshold = true ;
  } else
  if( xAxis < X_LOWER && ! xThreshold ) {
    prevPattern() ;
    Serial.println("Prev");
    xThreshold = true ;
  } else
  if( xAxis > X_LOWER && xAxis < X_UPPER ) {
    xThreshold = false ;
  }

  if( yAxis < Y_LOWER && ! yThreshold ) {
    if( brightness > 10 ) {
      brightness = min( brightness + 6, MAX_BRIGHTNESS );
    } else {
      brightness = min( brightness + 2, MAX_BRIGHTNESS );
    }
    LEDS.setBrightness(brightness);
    yThreshold = true ;
    Serial.print("Incr bright: ");
    Serial.println(brightness);
  } else
  if( yAxis > Y_UPPER && ! yThreshold ) {
    if( brightness < 10 ) {
      brightness = max( brightness - 2, MIN_BRIGHTNESS );
    } else {
      brightness = max( brightness - 6, MIN_BRIGHTNESS );
    }
    LEDS.setBrightness(brightness);
    yThreshold = true ;
    Serial.print("Decr bright: ");
    Serial.println(brightness);
  } else
  if( yAxis > Y_LOWER && yAxis < Y_UPPER ) {
    yThreshold = false ;
  }
}
